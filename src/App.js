...
class App extends Component {
  ...
  tick = () => {
    const {condition, seconds} = this.state
  ...
  clickImageItem = thumbnailUrl => {
    const {imageUrl, length} = this.state
    const updatedImageUrl = thumbnailUrl.replace('thumbnail-', '')
    if (imageUrl === updatedImageUrl) {
      const randomNumber = Math.ceil(Math.random() * length - 1)
      const randomImageUrl = imagesList[randomNumber].imageUrl
      this.setState(prevState => ({
  ...
  render() {
    const {seconds, score, initialTabId, imageUrl, condition} = this.state
    const filteredImagesList = imagesList.filter(
      each => each.category === initialTabId,
    )

    let bottomSection
    if (condition === false || seconds === 0) {
      bottomSection = (
        <GameOver
          score={score}
          ...
          <ul className="tab-items">
            {tabsList.map(tabDetails => (
              <TabItem
                ...
                isActive={initialTabId === tabDetails.tabId}
              />
            ))}
          </ul>
    ...
    return (
      <div className="bg-container">
        <Header seconds={seconds} score={score} />
        {bottomSection}
      </div>
    )
  }
